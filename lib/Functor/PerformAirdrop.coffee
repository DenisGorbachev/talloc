...
export default class PerformAirdrop extends Functor
  ...
  execute: ->
    ...
    message = @buildMessageForAirdropRecipients()
  ...
  buildMessageForAirdropRecipients: ->
    project = @db.Projects.findOne(slug: @projectUid)
    """
      #{project.description}

      #{project.name} is airdropping free #{@assetUid} tokens to their community members. Sign up at their airdrop page, complete easy tasks and submit your details to the airdrop page to receive free entries. Also, earn up to #{@referralLimit} entries by inviting your friends. Entries will be converted to #{@assetUid} token in the ratio of 10:1. The airdrop ends at #{@to}.
    """
  sendMessagesToAirdropFeeds: ->
